...
**Two-tier history.** In-session history (Gradio state, ephemeral) handles the current conversation. Semantic history (Chroma, persistent) retrieves relevant turns from _previous_ sessions. The current conversation is explicitly excluded from semantic retrieval to prevent the LLM seeing the same dialogue twice.
